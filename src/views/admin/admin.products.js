...
class AdminProduct extends Component {
    ...
    loadCategories = () => {
        getAllCategories()
            .then( result => {
                this.setState({categories : [...this.state.categories , ...result.map( c => c.name )  ] });
            })
            .catch ( err => {
                console.log(err);
            })
                                ...
                                <div className="table-responsive px-2">
                                    ...
                                    <tbody>
                                      { this.filterByCategory()
                                            .map( item => this.renderProductTableItem(item) )}                                      
                                    </tbody>
                                    </table>
                                </div>
                            </div>
    ...
    filterByCategory = () => {
        ...
        if( active_category == 'All'){
            ...
            return products
            .filter( p => p.category_name == active_category) 
        }
    ...
    renderProductTableItem = item => {
        return(
            <tr key={item._id}>
                <td>
                    <img src={Config.setImage(item.images[0])} height={60} className="mr-2"/>
                    { item.images.length > 1 && <img src={Config.setImage(item.images[1])} height={60}/>}
                </td>
                <td><h6 className="form-label mb-2">{item.name}</h6>
                {item.sizes.map( (size , i) => <span key={i} className="mr-2 border border-muted px-2 text-muted">{size.label}</span>)}
                </td>
                <td><h6 className="">{item.category_name}</h6></td>
                <td>
